...
const RestaurentPage=()=>{
    ...
    let GetRestaurentDetails=async ()=>{
        try{
        let URL=`https://zomoto-clone-backend-hgaz.onrender.com/api/Get-restaurent-Details-by-id/${id}`;
...
const GetMenuItems=async ()=>{
    let URL=`https://zomoto-clone-backend-hgaz.onrender.com/api/Get-menu-items-by-restaurent-id/${id}`;
    let response=await fetch(URL,{method:"GET"});
    let data=await response.json();
...
const MakePayment=async ()=>{
    let URL=`https://zomoto-clone-backend-hgaz.onrender.com/api/Create-order-id`;
   ...
   var options = {
    ...
    "handler":async function (response){
        ...
        try {
               let URL=`https://zomoto-clone-backend-hgaz.onrender.com/api/verify-payment`;
...
const GetLoginResult=()=>{
    ...
    Logindata &&
    SetLoginesult({...loginresult,address:Logindata.address,Name:Logindata.Name,Email:Logindata.email})
    }
    useEffect(()=>{
                ...
                <div className="modal-dialog">
                  <div className="modal-content">
                    <div className="modal-header">
                      ...
                      <button type="button" className="btn-close " data-bs-dismiss="modal" aria-label="Close" onClick={()=>{SetTotalprice(0)}}></button>
                    </div>
              ...
              <div className="modal" tabIndex="-1" id="UserForm">
                <div className="modal-dialog">
                  <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <p>Full Name:</p>
                        <input type="text" name="" id="fullname" className="w-100 modal-input rounded-2 mb-4" value={loginresult.Name} onChange={()=>{}} readOnly/>
                        <p>Email</p>
                        <input type="text" name="" id="Email"className="w-100 modal-input rounded-2 mb-4" value={loginresult.Email} onChange={()=>{}} readOnly/>
                        <p>Address:</p>
                        <textarea name="Address" id="" cols="30" rows="5" className="w-100 modal-input rounded-2 mb-4" value={loginresult.address} onChange={()=>{}} readOnly></textarea>
